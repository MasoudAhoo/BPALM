...
for i=1:1
    ...
    beta2  = 1;
    L0(1)  = 2/(beta1*beta2);
    gamma1 = 1/L0(1)-eps; % stepsize for U
    ...
    for j = 1:length(Slist) 
        ...
        switch Slist{j}
            ...
            case 'BPALM'
                fprintf('Running continuation-BPALM ...\n')
                contOpts.x0        = x0;
                contOpts.X         = X;
                contOpts.func      = func;
                contOpts.kernel    = kernel;
                contOpts.subprob   = subprob;
                contOpts.options   = options;
                contOpts.method    = 'BPALM';
                contOpts.contIter      = contIter; 
                contOpts.contTimeLimit = contTimeLimit;
                contOpts.c         = c;
                contOpts.lambda    = lambda;
                contOpts.stop_crit = stop_crit;
                tic
                [x,f,output] = continuation(contOpts);         
                toc
                F           = output.F;
                F_best      = F(1);
                fun_BPALM   = F(end);
                t_BPALM     = output.T;
                nfunc_BPALM = output.Nfunc;
                f_eval{j}   = F';
                
            case 'A-BPALM1'
                fprintf('Running continuation-A-BPALM1 ...\n')
                contOpts.x0        = x0;
                contOpts.X         = X;
                contOpts.func      = func;
                contOpts.kernel    = kernel;
                contOpts.subprob   = subprob;
                contOpts.method    = 'A-BPALM1';
                contOpts.contIter  = contIter; 
                contOpts.c         = c;
                contOpts.lambda    = lambda;
                contOpts.stop_crit = stop_crit;
                ...
                fun_A_BPALM   = F(end);
                t_BPALM       = output.T;
                nfunc_A_BPALM = output.Nfunc;
                f_eval{j}     = F';
                
            case 'A-BPALM2'
                fprintf('Running continuation-A-BPALM2 ...\n')
                contOpts.x0        = x0;
                contOpts.X         = X;
                contOpts.func      = func;
                contOpts.kernel    = kernel;
                contOpts.subprob   = subprob;
                contOpts.method    = 'A-BPALM2';
                contOpts.contIter  = contIter; 
                contOpts.c         = c;
                contOpts.lambda    = lambda;
                contOpts.stop_crit = stop_crit;
